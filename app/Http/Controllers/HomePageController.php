...
class HomePageController extends Controller
{
    ...
    // service boxes save to database
    public function saveOportunityBox(Request $requ){
        $existChk = HomepageModule::where(['section'=>$requ->section,'title'=>$requ->boxHeading])->first();
        if(!empty($existChk)):
    ...
    public function updateOportunityBox(Request $requ){
        $section = HomepageModule::find($requ->boxId);
        if(empty($section)):
            return back()->with('error','Sorry! No data found');
        else:
    ...
    public function editOportunityBox($id){
        $existChk = HomepageModule::find($id);
        return view('superadmin.editOportunityBox',['oprotunity'=>$existChk]);
    }

    public function delOportunityBox($id){
        $existChk = HomepageModule::find($id);
        if(empty($existChk)):
            return back()->with('error','Sorry! No data found');
        else:
